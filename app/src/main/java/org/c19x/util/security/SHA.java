...
/**
 * FIPS 180-2 implementation of SHA-512, adapted from the Bouncy Castle implementation
 * to optimise performance where possible.
 * <p>
 */
public class SHA {
    ...
    private final byte[] digest = new byte[DIGEST_LENGTH];
     ...
     */
    private long H1 = 0x6a09e667f3bcc908L;
    private long H2 = 0xbb67ae8584caa73bL;
    private long H3 = 0x3c6ef372fe94f82bL;
    private long H4 = 0xa54ff53a5f1d36f1L;
    private long H5 = 0x510e527fade682d1L;
    private long H6 = 0x9b05688c2b3e6c1fL;
    private long H7 = 0x1f83d9abfb41bd6bL;
    private long H8 = 0x5be0cd19137e2179L;
     ...
     */
    private final static long K[] = {0x428a2f98d728ae22L, 0x7137449123ef65cdL, 0xb5c0fbcfec4d3b2fL, 0xe9b5dba58189dbbcL, 0x3956c25bf348b538L, 0x59f111f1b605d019L,
    ...
    /**
     * Compute SHA-512 hash of data. This is not a thread-safe method, but it has been optimised for
     * speed.
     *
     * @param data Data for hashing.
     * @return SHA-512 hash of the data.
     */
    ...
    /**
     * Verify SHA-512 hash of data. This is not a thread-safe method, but it has been optimised for
     * speed.
     *
     * @param data           Data for hashing.
     * @param expectedDigest Expected digest for the data.
     * @return SHA-512 hash of the data.
     */
    public boolean verify(final byte[] data, final byte[] expectedDigest) {
        reset();
        update(data, 0, data.length);
        doFinal(digest, 0);
        return Arrays.equals(digest, expectedDigest);
    }
    ...
    private static long bigEndianToLong(byte[] bs, int off) {
        int hi = bigEndianToInt(bs, off);
        int lo = bigEndianToInt(bs, off + 4);
        return ((long) (hi & 0xffffffffL) << 32) | (long) (lo & 0xffffffffL);
    }

    private static int bigEndianToInt(byte[] bs, int off) {
        int n = bs[off] << 24;
        n |= (bs[++off] & 0xff) << 16;
        n |= (bs[++off] & 0xff) << 8;
        n |= (bs[++off] & 0xff);
    ...
    private static void longToBigEndian(long n, byte[] bs, int off) {
