...
//            AppDelegate.getNetworkClient().checkConnectionToServer(r -> {
//                if (r.getValue()) {
//                    // Sync time with server
//                    AppDelegate.getTimestamp().getTime();
...
//                AppDelegate.getDeviceRegistration().isRegistered();
//        final boolean bluetoothBeaconComplete =
//                (!AppDelegate.getBeaconTransmitter().isSupported() || (AppDelegate.getBeaconTransmitter().isSupported() && AppDelegate.getBeaconTransmitter().isStarted())) &&
//                        (!AppDelegate.getBeaconReceiver().isSupported() || (AppDelegate.getBeaconReceiver().isSupported() && AppDelegate.getBeaconReceiver().isStarted()));
//        final boolean dataDownloadComplete =
//                AppDelegate.getGlobalStatusLog().getTimestamp() > 0;
//
...
//                AppDelegate.getBeaconTransmitter().isSupported(),
//                AppDelegate.getBeaconTransmitter().isStarted(),
//                AppDelegate.getBeaconReceiver().isSupported(),
//                AppDelegate.getBeaconReceiver().isStarted(),
...
//
//                AppDelegate.getDeviceRegistration().removeListener(this);
//            }
//        };
//
//        final DeviceRegistration deviceRegistration = AppDelegate.getDeviceRegistration();
...
//                checkBox.setChecked(success);
//                checkBox.setTextColor(getResources().getColor(success ? R.color.colorGreen : R.color.colorRed, null));
//                AppDelegate.getGlobalStatusLog().removeListener(this);
//            }
//        };
//
//        AppDelegate.getGlobalStatusLog().addListener(globalStatusLogListener);
//        final long timestamp = AppDelegate.getGlobalStatusLog().getTimestamp();
//        if (timestamp == 0) {
...
//        if (!AppDelegate.getBluetoothStateMonitor().isSupported()) {
...
//
//        // Enable bluetooth
//        if (!AppDelegate.getBluetoothStateMonitor().isEnabled()) {
...
//            intent.putExtra(BeaconService.keyId, AppDelegate.getAliasIdentifier());
//            intent.putExtra(BeaconService.keyReceiverOnDuration, AppDelegate.getGlobalStatusLog().getBeaconReceiverOnDuration());
//            intent.putExtra(BeaconService.keyReceiverOffDuration, AppDelegate.getGlobalStatusLog().getBeaconReceiverOffDuration());
